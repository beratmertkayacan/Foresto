...
def eoq_hesapla(urun_id: int, db: Session) -> dict:
    ...
    S = urun.siparis_maliyeti_tl or 0
    maliyet = urun.maliyet_fiyati or 0
    tutma_oran = urun.yillik_tutma_maliyeti_oran or 0
    H = maliyet * tutma_oran

    if S <= 0 or H <= 0:
        return {"hata": "EOQ için sipariş maliyeti ve tutma maliyeti tanımlı olmalı"}
    ...
    mevcut = urun.mevcut_stok if urun.mevcut_stok is not None else 0
    min_stok = urun.min_stok_seviyesi if urun.min_stok_seviyesi is not None else 0
    esik = max(reorder_point, min_stok)
    ...
    return {
        ...
        "mevcut_stok": mevcut,
        "siparis_gerekli_mi": mevcut <= esik,
        "aciklama": {
