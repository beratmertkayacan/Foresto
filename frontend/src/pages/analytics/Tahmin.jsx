...
function tahminGuvenilirligi(tahmin, gecmisLen) {
    if (tahmin?.fallback) return { baslik: 'Sınırlı', alt: 'Daha fazla satış verisi gerekli' }
    if (gecmisLen < 14) return { baslik: 'Orta', alt: 'Veri birikiyor' }
    return { baslik: 'Yüksek', alt: 'Satış geçmişi yeterli' }
}
...
export default function Tahmin() {
    ...
    const [sunucuHatasi, setSunucuHatasi] = useState(false)
    const [veriEksikMesaji, setVeriEksikMesaji] = useState("")
    ...
    useEffect(() => {
        axios.get("/urunler/").then(r => setUrunler(r.data)).catch(() => setUrunler([]))
    }, [])
    ...
    useEffect(() => {
        ...
        const gecmisIstegi = axios.get(
            `/analitik/gecmis/${seciliUrun.urun_id}?gun=365`
        ).then(r => setGecmis(Array.isArray(r.data) ? r.data : [])).catch(() => setGecmis([]))

        const tahminIstegi = axios.get(
            `/analitik/tahmin/${seciliUrun.urun_id}?gun=${aralik}`
        ).then(r => {
            const veri = r.data
            if (veri?.hata) {
                setVeriEksikMesaji(veri.hata)
                setTahmin(null)
            } else {
                setTahmin(veri)
    ...
    const gunlukOrt = tahmin?.gunluk_ortalama ?? 0
    const mevcutStok = seciliUrun?.mevcut_stok ?? 0
    const tukenmeGun = tahmin && seciliUrun && gunlukOrt > 0 && mevcutStok >= 0
        ? Math.floor(mevcutStok / gunlukOrt)
        : null

    if (urunler.length === 0) {
        return (
            <div className="space-y-6">
                <h1 className="text-2xl font-bold ss-ink ss-metric">Talep Tahmini</h1>
                <EmptyState
                    icon="📈"
                    title="Tahmin için veri yok"
                    description="Talep tahmini yapabilmek için önce ürün ekleyin ve stok çıkış hareketleri kaydedin."
    ...
    {
        !seciliUrun && (
            <div className="flex flex-col items-center justify-center h-96 ss-card ss-card--hover border border-dashed border-gray-300 dark:border-gray-700">
                <TrendingUp size={48} className="text-gray-300 mb-4" />
                <p className="text-gray-400 text-lg font-medium">Talep tahminine başlayın</p>
                <p className="text-gray-300 text-sm mt-1">Ürün arayın veya listeden seçin</p>
            </div>
        )
    }
    ...
    {/* Ağ / sunucu hatası */}
    {
        seciliUrun && !yukleniyor && !tahmin && sunucuHatasi && (
            <div className="flex flex-col items-center justify-center h-64 ss-card ss-card--hover border border-dashed border-red-200 dark:border-red-800/50">
                <AlertTriangle size={32} className="text-red-400 mb-3" />
                <p className="text-gray-700 dark:text-gray-200 font-semibold">Sunucuya bağlanılamadı</p>
                <p className="text-sm text-gray-400 mt-1 mb-4">İnternet bağlantınızı kontrol edip tekrar deneyin.</p>
        ...
        seciliUrun && !yukleniyor && !tahmin && !sunucuHatasi && (
            <EmptyState
                ...
                description={
                    veriEksikMesaji ||
                    (gecmis.length === 0
                        ? "Bu ürün için henüz stok çıkış hareketi yok. Hareket ekledikten sonra tekrar deneyin."
                        : "Satış geçmişi tahmin için yeterli değil. Daha fazla çıkış hareketi ekleyin.")
                }
                ctaLabel="Hareket ekle"
                ctaTo="/hareketler"
            />
        )
    }
        ...
        seciliUrun && !yukleniyor && tahmin && (
            <>
                ...
                <div className="grid grid-cols-4 gap-4">
                    <div className="ss-card ss-card--hover p-5">
                        <div className="flex items-center justify-between mb-3">
                            <p className="ss-label">{aralik} Gün Tahmini</p>
                            <div className="w-8 h-8 bg-blue-50 dark:bg-blue-900/30 rounded-lg flex items-center justify-center">
                                <TrendingUp size={16} className="text-blue-600" />
                            </div>
                        </div>
                        <p className="text-3xl font-bold ss-ink ss-metric">{tahmin.toplam_tahmini_talep ?? 0}</p>
                        <p className="text-xs text-gray-400 mt-1">{seciliUrun.birim} tahmini satış</p>
                    </div>

                    <div className="ss-card ss-card--hover p-5">
                        <div className="flex items-center justify-between mb-3">
                            <p className="ss-label">Günlük Ortalama</p>
                            <div className="w-8 h-8 bg-purple-50 dark:bg-purple-900/30 rounded-lg flex items-center justify-center">
                                <Package size={16} className="text-purple-600" />
                            </div>
                        </div>
                        <p className="text-3xl font-bold ss-ink ss-metric">{tahmin.gunluk_ortalama ?? 0}</p>
                        <p className="text-xs text-gray-400 mt-1">{seciliUrun.birim} / gün</p>
                    </div>

                    <div className={`rounded-2xl p-5 shadow-sm border ${tukenmeGun == null ? "bg-gray-50 dark:bg-gray-800 border-gray-200" : tukenmeGun <= 7 ? "bg-red-50 dark:bg-red-900/20 border-red-200" : tukenmeGun <= 14 ? "bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200" : "bg-green-50 dark:bg-green-900/20 border-green-200"}`}>
                        <div className="flex items-center justify-between mb-3">
                            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Stok Tükenme</p>
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${tukenmeGun == null ? "bg-gray-100" : tukenmeGun <= 7 ? "bg-red-100" : tukenmeGun <= 14 ? "bg-yellow-100" : "bg-green-100"}`}>
                                <AlertTriangle size={16} className={tukenmeGun == null ? "text-gray-400" : tukenmeGun <= 7 ? "text-red-600" : tukenmeGun <= 14 ? "text-yellow-600" : "text-green-600"} />
                            </div>
                        </div>
                        <p className={`text-3xl font-bold ${tukenmeGun == null ? "text-gray-500" : tukenmeGun <= 7 ? "text-red-600" : tukenmeGun <= 14 ? "text-yellow-600" : "text-green-600"}`}>
                            {tukenmeGun == null ? "—" : `${tukenmeGun} gün`}
                        </p>
                        <p className="text-xs text-gray-500 mt-1">Bu hızda tükenme süresi</p>
                    </div>

                    {(() => {
                        const g = tahminGuvenilirligi(tahmin, gecmis.length)
                        return (
                    <div className="ss-card ss-card--hover p-5">
                        <div className="flex items-center justify-between mb-3">
                            <p className="ss-label">Tahmin Güvenilirliği</p>
                        ...
                        <p className={`text-3xl font-bold ${tahmin?.fallback ? "text-amber-500" : "ss-ink ss-metric"}`}>{g.baslik}</p>
                        <p className="text-xs text-gray-400 mt-1">{g.alt}</p>
                    </div>
                        )
                    })()}
                </div>
                ...
                {/* Grafik */}
                <div className="ss-card ss-card--hover p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="ss-card ss-card--hover p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                        <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4 flex items-center gap-2">
                            <Clock size={16} className="text-blue-500" /> Tahmin Özeti
                        </h3>
                        <div className="space-y-4">
                            {[
                                { label: "Dönem toplamı", value: `${tahmin.toplam_tahmini_talep ?? 0} ${seciliUrun.birim}` },
                                { label: "Günlük ortalama", value: `${tahmin.gunluk_ortalama ?? 0} ${seciliUrun.birim}` },
                                { label: "Tahmin süresi", value: `${aralik} gün` },
                            ].map(item => (
    ...
    {yukleniyor && <PageLoader fullHeight={false} label="Talep tahmini hesaplanıyor…" />}
        </div >
    )
}