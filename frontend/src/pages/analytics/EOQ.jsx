...
export default function EOQ() {
    ...
    useEffect(() => {
        ...
        setYukleniyor(true)
        setEoqData(null)
        axios.get(`/analitik/eoq/${seciliUrun.urun_id}`)
            .then(r => { setEoqData(r.data?.hata ? null : r.data); setYukleniyor(false) })
            .catch(() => { setEoqData(null); setYukleniyor(false) })
    }, [seciliUrun])
    ...
    const maliyetEgrisiVerisi = eoqData?.aciklama ? (() => {
        const D = eoqData.aciklama.D_yillik_talep || 0
        const S = eoqData.aciklama.S_siparis_maliyeti || 0
        const H = eoqData.aciklama.H_tutma_maliyeti || 0
        const Q = eoqData.optimal_siparis_miktari || 1
        if (D <= 0 || H <= 0 || Q <= 0) return []
        ...
        for (let q = minQ; q <= maxQ; q += step) {
            const siparisM = q > 0 ? Math.round((D / q) * S) : 0
            const tutmaM = Math.round((q / 2) * H)
            points.push({ miktar: q, siparisM, tutmaM, toplamM: siparisM + tutmaM })
        }
        return points
    })() : []

    // Türetilmiş değerler
    const hesapla = eoqData?.aciklama ? (() => {
        const D = eoqData.aciklama.D_yillik_talep || 0
        const S = eoqData.aciklama.S_siparis_maliyeti || 0
        const H = eoqData.aciklama.H_tutma_maliyeti || 0
        const Q = eoqData.optimal_siparis_miktari || 0
        if (D <= 0 || Q <= 0) return null
        const gunlukTalep = D / 365
        const yillikSiparisSayisi = D / Q
        const siparisAraligiGun = yillikSiparisSayisi > 0 ? Math.round(365 / yillikSiparisSayisi) : 0
    ...
    if (urunler.length === 0) {
        return (
            <div className="space-y-6">
                <h1 className="text-2xl font-bold ss-ink ss-metric">Sipariş Önerisi</h1>
                <EmptyState
                    icon="📐"
                    title="Sipariş önerisi için veri yok"
                    description="Sipariş önerisi için önce ürün ekleyin ve yeterli satış (çıkış) hareketi kaydedin."
        ...
        <div className="space-y-6">
            ...
            {!seciliUrun && (
                <div className="flex flex-col items-center justify-center h-96 ss-card ss-card--hover border border-dashed border-gray-300 dark:border-gray-700">
                    ...
                    <p className="text-gray-400 text-lg font-medium">Sipariş önerisine başlayın</p>
                    <p className="text-gray-300 text-sm mt-1">Ürün arayın veya listeden seçin</p>
                </div>
            )}

            {/* Yükleniyor */}
            {yukleniyor && <PageLoader fullHeight={false} label="Sipariş önerisi hesaplanıyor…" />}

            {seciliUrun && !yukleniyor && !eoqData && (
                <EmptyState
                    compact
                    icon="📐"
                    title="Sipariş önerisi hesaplanamadı"
                    description="Bu ürün için yeterli çıkış hareketi veya maliyet bilgisi yok. Stok çıkışı ve sipariş/tutma maliyetlerini tanımlayın."
            ...
            {seciliUrun && !yukleniyor && eoqData && hesapla && (
                <>
                    ...
                    <div className="grid grid-cols-4 gap-4">
                        <div className="ss-card ss-card--hover p-5">
                            <div className="flex items-center justify-between mb-3">
                                <p className="ss-label">Optimum Sipariş</p>
                                <div className="w-8 h-8 bg-blue-50 dark:bg-blue-900/30 rounded-lg flex items-center justify-center">
                                    <ShoppingCart size={16} className="text-blue-600" />
                                </div>
                            </div>
                            <p className="text-3xl font-bold ss-ink ss-metric">{eoqData.optimal_siparis_miktari}</p>
                            <p className="text-xs text-gray-400 mt-1">{seciliUrun.birim} / sipariş</p>
                        </div>

                        <div className="ss-card ss-card--hover p-5">
                            <div className="flex items-center justify-between mb-3">
                                <p className="ss-label">Sipariş Noktası</p>
                                <div className="w-8 h-8 bg-orange-50 dark:bg-orange-900/30 rounded-lg flex items-center justify-center">
                                    <AlertTriangle size={16} className="text-orange-500" />
                                </div>
                            </div>
                            <p className="text-3xl font-bold ss-ink ss-metric">{eoqData.siparis_noktasi}</p>
                            <p className="text-xs text-gray-400 mt-1">{seciliUrun.birim} altında sipariş ver</p>
                        </div>

                        <div className="ss-card ss-card--hover p-5">
                            <div className="flex items-center justify-between mb-3">
                                <p className="ss-label">Güvenlik Stoğu</p>
                                <div className="w-8 h-8 bg-purple-50 dark:bg-purple-900/30 rounded-lg flex items-center justify-center">
                                    <Package size={16} className="text-purple-600" />
                                </div>
                            </div>
                            <p className="text-3xl font-bold ss-ink ss-metric">{eoqData.guvenlik_stogu}</p>
                    ...
                    <div className="ss-card ss-card--hover p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                ...
                                <p className="text-sm text-gray-400">Önerilen miktar ({eoqData.optimal_siparis_miktari} {seciliUrun.birim}) yıllık maliyeti en düşük noktada</p>
                            </div>
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <LineChart data={maliyetEgrisiVerisi} margin={{ top: 5, right: 20, left: 10, bottom: 20 }}>
                                ...
                                <ReferenceLine
                                    ...
                                    stroke="#8b5cf6"
                                    strokeDasharray="5 5"
                                    label={{ value: `Öneri: ${eoqData.optimal_siparis_miktari}`, fill: "#8b5cf6", fontSize: 11, position: "top" }}
                                />
                        ...
                        <div className="flex items-center justify-between mb-2">
                            <div>
                                ...
                                <p className="text-sm text-gray-400 mt-0.5">
                                    ...
                                    {hesapla.teminSuresi} gün sonra {eoqData.optimal_siparis_miktari} {seciliUrun.birim} teslim alınır.
                                </p>
                            </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="ss-card ss-card--hover p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4 flex items-center gap-2">
                                <TrendingDown size={16} className="text-purple-500" /> Hesap Detayları
                            </h3>
                            <div className="space-y-3">
                                {[
                                    { label: "Yıllık Talep", value: `${Math.round(eoqData.aciklama.D_yillik_talep).toLocaleString("tr-TR")} ${seciliUrun.birim}` },
                                    { label: "Sipariş Maliyeti", value: `₺${eoqData.aciklama.S_siparis_maliyeti}` },
                                    { label: "Birim Tutma Maliyeti", value: `₺${eoqData.aciklama.H_tutma_maliyeti}` },
