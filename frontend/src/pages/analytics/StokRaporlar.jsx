...
export default function StokRaporlar() {
    ...
    useEffect(() => {
        ...
        const yukle = async () => {
            try {
                const ur = await axios.get("/urunler/")
                setUrunler(ur.data)
            } catch { /* ürünler yüklenemedi */ }

            try {
                const kr = await axios.get("/urunler/kritik")
                setKritikler(kr.data)
            } catch { /* kritikler yüklenemedi */ }

            try {
                const db = await axios.get("/analitik/dashboard")
    ...
    const abcGrafikVeri = abcOzet.map(o => ({ name: `Grup ${o.sinif}`, deger: Math.round(o.deger), adet: o.adet }))
    ...
    const csvIndir = () => {
        const satirlar = [
            ["Öncelik", "Ürün Adı", "Kategori", "Mevcut Stok", "Birim", "Stok Değeri (₺)", "Kümülatif %"],
        ...
        const a = document.createElement("a"); a.href = url; a.download = "urun_onceliklendirme.csv"; a.click()
    }
    ...
    const kritikCsvIndir = () => {
        const satirlar = [
            ...
            ...kritikler.map(u => {
                const eksik = (u.min_stok_seviyesi ?? 0) - (u.mevcut_stok ?? 0)
                return [u.urun_adi, u.kategori, u.mevcut_stok ?? 0, u.min_stok_seviyesi ?? 0, u.birim, eksik.toFixed(1)]
            })
        ]
        const csv = satirlar.map(r => r.join(",")).join("\n")
        const blob = new Blob(["﻿" + csv], { type: "text/csv;charset=utf-8;" })
    ...
    if (yukleniyor) return <PageLoader label="Raporlar yükleniyor…" />

    if (urunler.length === 0) {
        return (
            <div className="space-y-6">
                <h1 className="text-2xl font-bold ss-ink ss-metric">Stok Analizi & Raporlar</h1>
                <EmptyState
                    icon="📊"
                    title="Raporlar için veri yok"
                    description="Ürün ekledikten sonra önceliklendirme, kategori dağılımı ve kritik stok raporları burada görünecek."
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-xl w-fit">
                {[{ key: "abc", label: "Ürün Önceliklendirme" }, { key: "raporlar", label: "Raporlar" }].map(t => (
            ...
            {aktifTab === "abc" && (
                <>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        {abcOzet.map(o => (
                            <div key={o.sinif} className="ss-card ss-card--hover p-5">
                                <div className="flex items-center justify-between mb-3">
                                    <AbcRozeti sinif={o.sinif} />
                                    <span className="text-xs text-gray-400">{o.adet} ürün</span>
                                </div>
                                <p className="text-2xl font-bold ss-ink ss-metric">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="ss-card ss-card--hover p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Stok Değerine Göre Dağılım</h3>
                    ...
                    {/* ABC Tablosu */}
                    <div className="ss-card ss-card--hover shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-gray-50 dark:bg-gray-700/50">
                                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Öncelik</th>
                ...
                <>
                    ...
                    <div className="grid grid-cols-4 gap-4">
                        {[
                            ...
                            <div key={k.label} className="ss-card ss-card--hover p-5">
                                <div className="flex items-center justify-between mb-3">
                                    <p className="ss-label">{k.label}</p>
                                    <div className={`w-8 h-8 ${k.renk} rounded-lg flex items-center justify-center`}>{k.ikon}</div>
                                </div>
                                <p className="text-2xl font-bold ss-ink ss-metric">{k.value}</p>
                            </div>
                        ))}
                    </div>
