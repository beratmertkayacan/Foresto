...
export default function Hareketler() {
    ...
    const veriYukle = () => {
        Promise.all([
            axios.get("/hareketler/?limit=30000"),
            axios.get("/urunler/"),
            axios.get("/hareketler/toplam"),
        ]).then(([hr, ur, top]) => {
    ...
    const kaydet = async () => {
        ...
        try {
            if (duzenleMe) {
                await axios.put(`/hareketler/${duzenleMe}`, {
    ...
    const sil = async (id) => {
        try {
            await axios.delete(`/hareketler/${id}`)
            setSilOnay(null)
    ...
    const filtreliHareketler = hareketler
        .filter(h => {
            ...
            const hTarih      = h.tarih.slice(0, 10)
            const tarihTamam  = (!tarihBaslangic || hTarih >= tarihBaslangic) &&
                                (!tarihBitis     || hTarih <= tarihBitis)
            return aramaTamam && tipTamam && tarihTamam
        })
        .sort((a, b) => {
            if (siralama === "tarih_desc") return b.tarih.slice(0,10).localeCompare(a.tarih.slice(0,10)) || b.hareket_id - a.hareket_id
            if (siralama === "tarih_asc") return a.tarih.slice(0,10).localeCompare(b.tarih.slice(0,10)) || a.hareket_id - b.hareket_id
    ...
    const donemGiris = hareketler.filter(h => h.tarih.slice(0,10) >= kartBaslangic && h.tarih.slice(0,10) <= kartBitis && h.hareket_tipi === "giris")
    const donemCikis = hareketler.filter(h => h.tarih.slice(0,10) >= kartBaslangic && h.tarih.slice(0,10) <= kartBitis && h.hareket_tipi === "cikis")
    ...
    const grafikVerisi = (() => {
        ...
        const gunlukVeri = (gunSayisi) => {
            ...
            for (let i = gunSayisi - 1; i >= 0; i--) {
                ...
                const tarih = fmt(d)
                const gun = hareketler.filter(h => h.tarih.slice(0, 10) === tarih)
        ...
        const haftalikVeri = (haftaSayisi) => {
            ...
            for (let i = haftaSayisi - 1; i >= 0; i--) {
                ...
                const hafta = hareketler.filter(h => h.tarih.slice(0,10) >= bas && h.tarih.slice(0,10) <= son)
        ...
        const aylikVeri = (aySayisi) => {
            ...
            for (let i = aySayisi - 1; i >= 0; i--) {
                ...
                const ay = hareketler.filter(h => h.tarih.slice(0,10) >= ayBas && h.tarih.slice(0,10) <= fmt(ayson))
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-end justify-between">
                <div className="flex items-end gap-6">
                    <div>
                        <h1 className="text-2xl font-bold ss-ink ss-metric">Stok Hareketleri</h1>
            ...
            {!yukleniyor && hareketler.length === 0 && (
                <EmptyState
                    icon="🔄"
                    title={urunler.length === 0 ? "Henüz hareket yok" : "İlk stok hareketinizi ekleyin"}
                    description={
                        urunler.length === 0
                            ? "Hareket kaydı için önce ürün eklemeniz gerekir."
                            : "Giriş veya çıkış hareketi ekleyerek grafik ve listeleri doldurun."
                    }
                    ctaLabel={urunler.length === 0 ? "Ürünlere git" : "Hareket ekle"}
                    ctaTo={urunler.length === 0 ? "/urunler" : undefined}
                    onCtaClick={urunler.length > 0 ? () => panelAc() : undefined}
                />
            )}
            ...
            <div className="flex items-center gap-2 flex-wrap">
                <div className="flex gap-1 ss-card ss-card--static p-1">
                    {[
                        ["bugun", "Bugün"],
                        ["dun",   "Dün"],
                        ["hafta", "Son 7 Gün"],
            ...
            <div className="grid grid-cols-4 gap-4 items-start">
                ...
                <div className="ss-card ss-card--hover p-5">
                    <div className="flex items-center justify-between mb-3">
                        <p className="ss-label">{donemEtiketi} — Hareketler</p>
                        <div className="w-8 h-8 bg-blue-50 dark:bg-blue-900/30 rounded-lg flex items-center justify-center">
                            <ArrowLeftRight size={15} className="text-blue-600" />
                        </div>
                    </div>
                    <div className="flex items-end gap-2 mb-1">
                        <p className="text-2xl font-bold ss-ink ss-metric">{bugunGiris.length + bugunCikis.length}</p>
                ...
                <div className="ss-card ss-card--hover p-5">
                    <div className="flex items-center justify-between mb-3">
                        <p className="ss-label">{donemEtiketi} — Alım</p>
                    ...
                    <p className="text-2xl font-bold ss-ink ss-metric mb-1">
                ...
                <div className="ss-card ss-card--hover p-5">
                    <div className="flex items-center justify-between mb-3">
                        <p className="ss-label">{donemEtiketi} — Satış</p>
                ...
                <div className="ss-card ss-card--hover p-5">
                    <div className="flex items-center justify-between mb-3">
                        <p className="ss-label">Tahmini {donemEtiketi} Kâr</p>
            ...
            <div className="-mx-8">
                <div className="ss-table-wrap border-y">
                                ...
                                {filtreliHareketler.slice(0, 100).map(h => {
                                    ...
                                    return (
                                        <tr key={h.hareket_id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                            <td className="px-3 py-2.5 text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap pl-8">{h.tarih.slice(0,10)}</td>
            ...
            {silOnay && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[70] flex items-center justify-center">
                    <div className="ss-card ss-card--hover shadow-2xl p-6 w-80">
            ...
            <div className={`fixed top-16 right-0 h-[calc(100vh-4rem)] z-[65] transition-all duration-300
                ...
                <div ref={panelRef}
                    className={`h-full w-[520px] ss-drawer shadow-[-12px_0_40px_rgba(0,0,0,0.14)] flex flex-col transition-transform duration-300 ease-out
                    ...
                    <div ref={formScrollRef} className="flex-1 overflow-y-auto px-7 py-6 space-y-6">
                        ...
                        {/* Hareket Tipi */}
                        <div>
                            <label className="block ss-label mb-2.5">Hareket Tipi</label>
                        ...
                        {/* Ürün */}
                        <div>
                            <label className="block ss-label mb-2.5">Ürün *</label>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="block ss-label mb-2.5">
                            ...
                            <div>
                                <label className="block ss-label mb-2.5">Birim Fiyat (₺)</label>
                        ...
                        {/* Tarih */}
                        <div>
                            <label className="block ss-label mb-2.5">Tarih *</label>
                        ...
                        {/* Fatura No */}
                        <div>
                            <label className="block ss-label mb-2.5">Fatura / İrsaliye No</label>
                        ...
                        {/* Açıklama */}
                        <div>
                            <label className="block ss-label mb-2.5">Açıklama / Not</label>
